/**
 * Compresses the cleaned port JSON data using gzip.
 * Reads data/ports.json and writes src/ports.data.json
...
async function compressJSON(): Promise<void> {
    const inputPath = path.join(__dirname, '../data/ports.json');
    const outputPath = path.join(__dirname, '../src/ports.data.json');
    ...
    console.log('Compression complete. Compressed data written to ports.data.json');
}
